...
abstract class REST_Controller extends CI_Controller {
     ...
     */
     protected function _auth_override_check()
     {
         ...
         $CL=mb_strtolower($this->router->class);
         $MT=mb_strtolower($this->router->method);
         $RM=mb_strtolower($this->request->method);

         // Check to see if the override array is even populated
         if ( ! empty($auth_override_class_method))
         {
             // Check for wildcard flag for rules for classes
             if ( ! empty($auth_override_class_method[$CL]['*'])) // Check for class overrides
             {
                 // No auth override found, prepare nothing but send back a TRUE override flag
                 if ($auth_override_class_method[$CL]['*'] === 'none')
                 {
                     return TRUE;
                 }

                 // Basic auth override found, prepare basic
                 if ($auth_override_class_method[$CL]['*'] === 'basic')
                 {
                     $this->_prepare_basic_auth();
                     return TRUE;
                 }
                 ...
                 if ($auth_override_class_method[$CL]['*'] === 'digest')
                 {
                     $this->_prepare_digest_auth();
                     return TRUE;
                 }
                 ...
                 if ($auth_override_class_method[$CL]['*'] === 'session')
                 {
                     $this->_check_php_session();
                     return TRUE;
                 }
                 ...
                 if ($auth_override_class_method[$CL]['*'] === 'whitelist')
                 {
                     $this->_check_whitelist_auth();
                     return TRUE;
                 }
             ...
             if ( ! empty($auth_override_class_method[$CL][$MT]))
             {
                 // None auth override found, prepare nothing but send back a TRUE override flag
                 if ($auth_override_class_method[$CL][$MT] === 'none')
                 {
                     return TRUE;
                 }

                 // Basic auth override found, prepare basic
                 if ($auth_override_class_method[$CL][$MT] === 'basic')
                 {
                     $this->_prepare_basic_auth();
                     return TRUE;
                 }
                 ...
                 // Digest auth override found, prepare digest
                 if ($auth_override_class_method[$CL][$MT] === 'digest')
                 {
                     $this->_prepare_digest_auth();
                     return TRUE;
                 }
                 ...
                 // Session auth override found, check session
                 if ($auth_override_class_method[$CL][$MT] === 'session')
                 {
                     $this->_check_php_session();
                     return TRUE;
                 }
                 ...
                 // Whitelist auth override found, check client's ip against config whitelist
                 if ($auth_override_class_method[$CL][$MT] === 'whitelist')
                 {
                     $this->_check_whitelist_auth();
                     return TRUE;
                 }
         ...
         {
             // check for wildcard flag for rules for classes
             if ( ! empty($auth_override_class_method_http[$CL]['*'][$RM]))
             {
                 // None auth override found, prepare nothing but send back a TRUE override flag
                 if ($auth_override_class_method_http[$CL]['*'][$RM] === 'none')
                 {
                     return TRUE;
                 }

                 // Basic auth override found, prepare basic
                 if ($auth_override_class_method_http[$CL]['*'][$RM] === 'basic')
                 {
                     $this->_prepare_basic_auth();
                     return TRUE;
                 }
                 ...
                 // Digest auth override found, prepare digest
                 if ($auth_override_class_method_http[$CL]['*'][$RM] === 'digest')
                 {
                     $this->_prepare_digest_auth();
                     return TRUE;
                 }
                 ...
                 // Session auth override found, check session
                 if ($auth_override_class_method_http[$CL]['*'][$RM] === 'session')
                 {
                     $this->_check_php_session();
                     return TRUE;
                 }
                 ...
                 // Whitelist auth override found, check client's ip against config whitelist
                 if ($auth_override_class_method_http[$CL]['*'][$RM] === 'whitelist')
                 {
                     $this->_check_whitelist_auth();
                     return TRUE;
                 }
             ...
             if ( ! empty($auth_override_class_method_http[$CL][$MT][$RM]))
             {
                 // None auth override found, prepare nothing but send back a TRUE override flag
                 if ($auth_override_class_method_http[$CL][$MT][$RM] === 'none')
                 {
                     return TRUE;
                 }

                 // Basic auth override found, prepare basic
                 if ($auth_override_class_method_http[$CL][$MT][$RM] === 'basic')
                 {
                     $this->_prepare_basic_auth();
                     return TRUE;
                 }
                 ...
                 // Digest auth override found, prepare digest
                 if ($auth_override_class_method_http[$CL][$MT][$RM] === 'digest')
                 {
                     $this->_prepare_digest_auth();
                     return TRUE;
                 }
                 ...
                 // Session auth override found, check session
                 if ($auth_override_class_method_http[$CL][$MT][$RM] === 'session')
                 {
                     $this->_check_php_session();
                     return TRUE;
                 }
                 ...
                 // Whitelist auth override found, check client's ip against config whitelist
                 if ($auth_override_class_method_http[$CL][$MT][$RM] === 'whitelist')
                 {
                     $this->_check_whitelist_auth();
                     return TRUE;
                 }
